...
execute if score @s enmity.math_f matches 1.. at @e[type=marker,tag=enmity.setup,limit=1] run setblock ~ -64 ~ oak_sign{Text1:'[{"text":"DPS: ","color":"gray"},{"score":{"name":"@e[type=zombie,tag=enmity.this,limit=1]","objective":"enmity.math_f"},"color":"white"}]'} replace
...
execute if score @s enmity.math_f matches 1.. at @e[type=marker,tag=enmity.setup,limit=1] run setblock ~ -64 ~ bedrock replace
