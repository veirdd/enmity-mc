...
execute unless score @s[tag=enmity.hit,type=!player] enmity.taken_dmg matches 0.. run scoreboard players set @s enmity.taken_dmg 0
scoreboard players add @s[tag=enmity.hit,type=!player] enmity.taken_dmg 1
execute if score @s[tag=enmity.hit,type=player] enmity.dmg_rdc matches 1.. run scoreboard players add @s enmity.taken_dmg 1
tag @s add enmity.this
execute at @e[type=marker,tag=enmity.setup,limit=1] run setblock ~ -64 ~ oak_sign{Text1:'{"score":{"name":"@e[type=!#enmity:not_living,tag=!enmity.invulnerable,tag=enmity.this,limit=1]","objective":"enmity.taken_dmg"},"color":"red"}'} replace
...
execute if entity @s[type=#enmity:no_damage_sound,tag=enmity.hit] at @s run function enmity:utility/damage_sound
