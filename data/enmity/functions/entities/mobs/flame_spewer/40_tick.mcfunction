...
execute unless entity @s[nbt={PersistenceRequired:1b}] unless entity @p[distance=..32] if predicate enmity:random/random_0.05 run function enmity:utility/despawn
execute unless entity @s[nbt={PersistenceRequired:1b}] unless entity @p[distance=..128] run function enmity:utility/despawn