execute positioned ~ ~-1 ~ run tag @p[distance=..24,gamemode=!spectator,scores={enmity.age=600..}] add enmity.homing_target
execute facing entity @a[tag=enmity.homing_target,limit=1] eyes run tp @s ^ ^ ^0.425
...
execute if score @s enmity.age matches 5.. positioned ^ ^ ^0.17 as @e[scores={enmity.iframes=0},type=!#enmity:not_living,tag=!enmity.target_dummy,tag=!enmity.projectile,dx=0,type=!wither] positioned ~-0.99 ~-0.99 ~-0.99 if entity @s[dx=0] run tag @s add enmity.hit
