...
execute as @a[tag=!enmity.give_instant_health,tag=!enmity.update_health] if score @s enmity.health < @s enmity.max_hp if entity @s[nbt={Inventory:[{Slot:10b,id:"minecraft:warped_fungus_on_a_stick",tag:{CustomModelData:52}}]}] run function enmity:utility/health_modification/increase_health_by_1
...
execute if score %death_mode enmity.value matches 1 as @a if score @s enmity.death_mode_regen_cd matches 0 if score @s enmity.food matches 20 run scoreboard players set @s enmity.death_mode_regen_cd 3
...
schedule function enmity:100_tick 5s