...
execute as @e[type=!#enmity:not_living,tag=enmity.hit,tag=!enmity.invulnerable,type=!player] store result score @s enmity.math_a run data get entity @s Health
scoreboard players operation @e[type=!#enmity:not_living,tag=enmity.hit,tag=!enmity.invulnerable,type=!player] enmity.math_b = @s enmity.dmg
execute as @e[type=!#enmity:not_living,tag=enmity.hit,tag=!enmity.invulnerable,type=!player] run scoreboard players operation @s enmity.math_b -= @s enmity.dmg_rdc
...
execute as @e[type=!#enmity:not_living,tag=enmity.hit,tag=!enmity.invulnerable,type=!player,type=!ender_dragon] if score @s enmity.math_a <= @s enmity.math_b run kill @s
execute as @e[type=!#enmity:not_living,tag=enmity.hit,tag=!enmity.invulnerable,type=!player] if score @s enmity.math_b matches ..0 run scoreboard players set @s enmity.math_b 1
