tellraw @s [{"text":"-==============[","color":"gray"},{"text":"ROTTEN LURE","color":"white","bold":true},{"text":"]==============-","color":"gray"}]

tellraw @s [{"text":"The Rotten Lure is used to summon the Giant.","color":"gray"}]
tellraw @s [{"text":"\nDescription:\n   The smell of death is strong enough to attract the greatest of the undead.\n   Can only be used at night.","color":"gray"}]
tellraw @s [{"text":"\nPrimary obtaining method:\n   Crafting:\n      [-][0][-]\n      [0][1][0]\n      [1][0][-]\n      0 - Rotten Flesh\n      1 - Bone","color":"gray"}]

tellraw @s [{"text":"-==============[","color":"gray"},{"text":" ◀ ","color":"white","bold":true,"hoverEvent":{"action":"show_text","value":{"text":"Back"}},"clickEvent":{"action":"run_command","value":"/trigger enmity.wiki set 113"}},{"text":"]=====[","color":"gray"},{"text":" ⌂ ","color":"white","bold":true,"hoverEvent":{"action":"show_text","value":{"text":"Home"}},"clickEvent":{"action":"run_command","value":"/trigger enmity.wiki"}},{"text":"]==============-","color":"gray"}]