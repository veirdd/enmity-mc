execute unless entity @s[tag=enmity.sensed_player] store result score @s enmity.math_a run fill ~-8 ~-8 ~-8 ~8 ~8 ~8 water[level=15] replace sculk_sensor[sculk_sensor_phase=active]
execute unless entity @s[tag=enmity.sensed_player] run fill ~-8 ~-8 ~-8 ~8 ~8 ~8 sculk_sensor[sculk_sensor_phase=active] replace water[level=15]
