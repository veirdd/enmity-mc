execute if block ~ ~ ~ lava if score @s enmity.cooldown matches 0 if predicate enmity:random/random_0.25 at @s if entity @p[distance=..32,gamemode=!creative,gamemode=!spectator] anchored eyes run function enmity:entities/mobs/flame_spewer/check_obstruction
