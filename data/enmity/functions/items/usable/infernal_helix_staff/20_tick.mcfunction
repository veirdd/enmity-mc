...
execute if score @s enmity.cooldown matches 2 if entity @e[type=#enmity:enemies,tag=!enmity.sentry,tag=!enmity.tamed,tag=!enmity.projectile,distance=..16] run function enmity:items/usable/infernal_helix_staff/summon_projectile