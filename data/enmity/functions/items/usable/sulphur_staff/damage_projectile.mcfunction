...
execute positioned ^ ^ ^60 positioned ~ ~20 ~ run summon marker ~ ~ ~ {Tags:["enmity.direction_anchor"]}
...
execute store result score @s enmity.math_a run scoreboard players operation @s enmity.math_d -= @s enmity.math_a
execute store result score @s enmity.math_b run scoreboard players operation @s enmity.math_e -= @s enmity.math_b
execute store result score @s enmity.math_c run scoreboard players operation @s enmity.math_f -= @s enmity.math_c
execute as @e[type=!#enmity:not_living,tag=!enmity.boss,tag=enmity.hit] store result entity @s Motion[0] double 0.01 run scoreboard players get @e[type=item,tag=enmity.this,limit=1] enmity.math_a
execute as @e[type=!#enmity:not_living,tag=!enmity.boss,tag=enmity.hit] store result entity @s Motion[1] double 0.01 run scoreboard players get @e[type=item,tag=enmity.this,limit=1] enmity.math_b
execute as @e[type=!#enmity:not_living,tag=!enmity.boss,tag=enmity.hit] store result entity @s Motion[2] double 0.01 run scoreboard players get @e[type=item,tag=enmity.this,limit=1] enmity.math_c
kill @e[type=marker,tag=enmity.direction_anchor]
tag @e[type=!#enmity:not_living,tag=enmity.hit] remove enmity.hit
function enmity:items/usable/sulphur_staff/die_projectile