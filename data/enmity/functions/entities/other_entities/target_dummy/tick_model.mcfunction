...
execute if entity @s[predicate=enmity:entity/is_on_ground] run tp @s ~ ~ ~ facing entity @p[gamemode=!spectator] feet
tp @e[type=zombie,tag=enmity.this,limit=1] ~ ~0.1 ~
execute unless entity @e[type=zombie,tag=enmity.this,limit=1] run kill @s
execute as @p[distance=..2,predicate=enmity:entity/is_sneaking] as @e[type=armor_stand,tag=enmity.this,limit=1] run kill @s
